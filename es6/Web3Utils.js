...
class Web3Utils {
    ...
    static ConvertEthUnit(value, fromUnit, toUnit) {
        let valueInWei = Web3.utils.toWei(value, fromUnit);
        return Web3.utils.fromWei(valueInWei, toUnit);
    }
    ...
    allEvents(contract) {
        return contract.events.allEvents(function (error, ev) {
            ...
            if (ev.type == "mined")
                console.log(`${ev.event}(${JSON.stringify(ev.returnValues)})`);
        });
    }

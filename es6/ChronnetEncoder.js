...
import ChronnetConstants from "./ChronnetConstants";

class ChronnetEncoder {
    static EncodeAddressId(networkId, addr) {
        if (typeof networkId != "number")
            throw Error("NetworkId must be a number");

        let versionId = "01";
        let networkIdHex = Utils.UInt16ToHex(networkId).slice(2);
        let len = (addr.slice(2).length / 2).toString(16);
        let addrBytes = Utils.RightPad(addr.toLowerCase(), 56).slice(2);
    ...
    static EncodeErc20Package(
        ...
        const pkg =
            ...
            ChronnetConstants.PACKAGE_MAGIC.slice(2) + // 4  b
            data.slice(2) + // 18 b
            Utils.NumberToBytes32(tokenQty).slice(2); // 32 b
        return pkg;
    }
    ...
    static DecodeErc20Package(pkg) {
        ...
        return {
            ...
            magicNumber: "0x" + pkgBytes.substr(3 * slotsize + 20, 8),
            data: "0x" + pkgBytes.substr(3 * slotsize + 28, 36),
            tokenQty: Utils.Bytes32ToNumber(
                "0x" + pkgBytes.substr(4 * slotsize)
            ),
        };
    }

    static EncodeErc721Package(
        originatorId,
        recipientId,
        tokenId,
        packageType,
        ...
        const pkg =
            ...
            ChronnetConstants.PACKAGE_MAGIC.slice(2) + // 4  b
            data.slice(2) + // 18 b
            Utils.NumberToBytes32(nftTokenId).slice(2) +
            Utils.AsciiToBytes(tokenUri).slice(2);
    ...
    static DecodeErc721Package(pkg) {
        ...
        return {
            ...
            magicNumber: "0x" + pkgBytes.substr(3 * slotsize + 20, 8),
            data: "0x" + pkgBytes.substr(3 * slotsize + 28, 36),
            nftTokenId: Utils.Bytes32ToNumber(
                "0x" + pkgBytes.substr(4 * slotsize, slotsize)
            ),
            tokenUri: Utils.BytesToAscii(
                Utils.ToBytes("0x" + pkgBytes.substr(5 * slotsize))
            ),
        };
    }
}

export default ChronnetEncoder;

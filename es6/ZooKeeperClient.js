...
class ZooKeeperClient {
    constructor(connectionString) {
        ...
        if (!this.zkConnection) {
            this.zkConnection =
                process.env.CHRONNET_ZookeeperConfig__Connection;
        }
    ...
    createAll = async (path, data, flags) => {
        ...
        while (index < path.length && index >= 0) {
            ...
            if (before?.length > 0 && (await this.exists(before)) === false)
                await this.zkClient.create(before);
        }
    ...
    listChildren = async (parentPath) => {
        try {
            if (await this.exists(parentPath))
                return await this.zkClient.get_children(parentPath);
        } catch (error) {
            console.error(error);
        }
    };
    ...
    listSortedChildrenPath = async (parentPath) => {
        try {
            ...
            return children?.sort(
                (a, b) =>
                    parseInt(a.substring(a.length - 10)) -
                    parseInt(b.substring(b.length - 10))
            );
        } catch (error) {
            console.error(error);
        }
...
export default ZooKeeperClient;

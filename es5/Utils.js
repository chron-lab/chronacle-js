...
var _base58Universal = require("base58-universal");
...
class Utils {
  ...
  static ToHex = bytes => {
    if (bytes) return "0x" + bytes.reduce((output, elem) => output + ("0" + elem.toString(16)).slice(-2), "");
  };
  static ToBytes = hex => {
  ...
  static Base58ToHex = base58 => {
    return Utils.ToHex((0, _base58Universal.decode)(base58));
  };
  static Base58ToUInt256 = base58 => {
    const hex = Utils.Base58ToHex(base58);
    return _ethers.ethers.BigNumber.from(hex);
  };
  static UInt256ToBase58 = uint256 => {
    const hex = _ethers.ethers.BigNumber.from(uint256.toString()).toHexString();

    return Utils.HexToBase58(hex);
  };
  static HexToBase58 = hex => {
    return (0, _base58Universal.encode)(Utils.ToBytes(hex));
  };
  static Awaitable = async fun => {
    if (fun.on) return await fun.on("receipt", receipt => {//console.log(receipt)
    });
    return fun;
  };
  static Delay = ms => {
  ...
  static Bytes32ToNumber = bytes32 => {
    return _ethers.ethers.BigNumber.from(bytes32);
  };
